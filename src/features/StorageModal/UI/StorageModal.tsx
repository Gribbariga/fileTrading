...
export const StorageModal = () => {
  const { isGuest } = storageSlice();

  return (
    <>
      <DialogRootSC>
        <Dialog.Trigger>
          <Button size={"3"} variant="ghost" color="gray" loading={false}>
            <DotsVerticalIcon />
          </Button>
        </Dialog.Trigger>
...
const { TabContentSC, DialogRootSC, TitleWrapperSC } = StorageModalStyle();
